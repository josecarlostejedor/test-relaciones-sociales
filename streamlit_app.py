...
# URL del logo (usamos la versión oficial que se ajusta a cabeceras)
LOGO_URL = "https://www.becaseuropa.es/img/logo_becas_europa.png"
...
# CABECERA: Imagen del logo apareciendo en Streamlit
...
if st.session_state.step == 1:
    ...
    with st.form("social_form"):
        ...
        if st.form_submit_button("Finalizar y Generar Informe"):
            # Cálculos de percentiles aproximados para la demo
    ...
    st.markdown(f'<div class="ref-box"><b>Referencia Metodológica Obligatoria:</b><br>"{BIBLIOGRAPHIC_REF}"</div>', unsafe_allow_html=True)
    
    st.subheader("📩 Enviar Informe Detallado al Tutor")
    
    # Construcción del cuerpo del email con los 4 percentiles detallados
    subject = f"INFORME PSICOPEDAGÓGICO: {st.session_state.student['nombre']} {st.session_state.student['apellidos']}"
    body = f"""
INFORME DE EVALUACIÓN PSICOPEDAGÓGICA COMPLETO
==============================================
Alumno: {st.session_state.student['nombre']} {st.session_state.student['apellidos']}
Fecha: {st.session_state.student.get('fecha', 'Hoy')}

1. RESULTADOS CUANTITATIVOS (Percentiles PC):
- Comprensión Auditiva (Listening): {st.session_state.listening_score} / 8

1. Confianza: Seguridad y respeto en la relación. PC= {st.session_state.pc_conf}
2. Comunicación: Calidad del intercambio verbal e emocional. PC= {st.session_state.pc_com}
3. Alienación: Sentimientos de aislamiento o rechazo. PC= {st.session_state.pc_ali}
4. Total: Balance general del apego (Confianza + Comunicación - Alienación). PC= {st.session_state.pc_total}

2. REFERENCIA TÉCNICA:
{BIBLIOGRAPHIC_REF}

Un percentil (PC) superior a 50 indica una puntuación por encima de la media del grupo normativo (16-17 años).
==============================================
    """
