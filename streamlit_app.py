...
# Estilos CSS para una apariencia profesional y limpia
st.markdown("""
<style>
...
# --- PASO 2: LISTENING ---
    ...
    if not st.session_state.show_questions:
        st.info("Escucha el audio completo. Una vez termines, pulsa el botón para responder.")
        if st.button("He escuchado el audio, responder preguntas"):
...
# --- PASO 3: TRANSICIÓN ---
    ...
    st.title("Fase 1 Finalizada")
    st.success(f"Puntuación Listening: {st.session_state.quiz_score}/10")
...
# --- PASO 4: IPPA ---
    ...
    st.caption("Valora del 1 (Nada) al 7 (Totalmente) según tu realidad.")
...
# --- PASO 5: RESULTADO FINAL (CANDIDATO) Y ENVÍO AUTOMÁTICO ---
    ...
    if not st.session_state.data_sent:
        with st.status("Enviando informe al tutor...", expanded=False) as status:
            # Simulación de Envío de Datos (Aquí se integraría un requests.post a un webhook de Zapier/Make)
            # data = { "tutor": TUTOR_EMAIL, "alumno": st.session_state.student, "listening": st.session_state.quiz_score, "pc": st.session_state.pc_total }
    ...
    # VISTA MINIMALISTA PARA EL CANDIDATO
    st.markdown("---")
    ...
    st.markdown("---")
    st.info("Gracias por participar. Tus resultados han sido registrados de forma segura y enviados a la coordinación del programa.")
