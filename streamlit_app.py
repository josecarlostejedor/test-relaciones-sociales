...
if st.session_state.step == 1:
    ...
    with st.form("social_form"):
        ...
        if st.form_submit_button("Finalizar y Generar Informe"):
            # Mock de cálculo para la versión Streamlit
            ...
            st.session_state.pc_ali = 100 - st.session_state.pc_total # Inverso aproximado
            st.session_state.step = 4
            st.rerun()

elif st.session_state.step == 4:
    st.title("Informe de Evaluación")
    ...
    with col2: st.markdown(f'<div class="metric-card"><small>APEGO (PC TOTAL)</small><h2>PC {st.session_state.pc_total}</h2></div>', unsafe_allow_html=True)
    
    st.markdown(f'<div class="ref-box"><b>Referencia Metodológica:</b><br>"{BIBLIOGRAPHIC_REF}"</div>', unsafe_allow_html=True)
    
    st.subheader("📩 Envío de Informe Detallado al Tutor")
    
    subject = f"Informe Detallado Evaluación: {st.session_state.student['nombre']} {st.session_state.student['apellidos']}"
    body = f"""
INFORME DE EVALUACIÓN PSICOPEDAGÓGICA (IPPA)
==============================================
...
- Confianza: PC {st.session_state.pc_conf}
- Comunicación: PC {st.session_state.pc_com}
- Alienación: PC {st.session_state.pc_ali}
- TOTAL APEGO: PC {st.session_state.pc_total}

2. DEFINICIONES DEL MANUAL APLICADAS:
- Confianza: Seguridad y respeto en la relación.
- Comunicación: Calidad del intercambio verbal y emocional.
- Alienación: Sentimientos de aislamiento, vergüenza o rechazo.
- Puntuación Total: Balance general del apego (Confianza + Comunicación - Alienación).
