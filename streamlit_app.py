...
<style>
    ...
    .ref-box { background-color: #f1f5f9; padding: 15px; border-radius: 10px; font-style: italic; font-size: 0.85rem; color: #475569; margin: 20px 0; border-left: 4px solid #94a3b8; }
</style>
...
if st.session_state.step == 1:
    ...
    with col2: st.markdown(f'<div class="metric-card"><small>APEGO (PC ESTIMADO)</small><h2>PC {st.session_state.pc_social}</h2></div>', unsafe_allow_html=True)
    
    st.markdown(f'<div class="ref-box">"{BIBLIOGRAPHIC_REF}"</div>', unsafe_allow_html=True)
    
    st.subheader("📩 Envío Manual Detallado")
    
    subject = f"Informe Detallado Becas Europa - {st.session_state.student['nombre']}"
    body = f"""
INFORME DE EVALUACIÓN PSICOPEDAGÓGICA (SISTEMA SIMPLIFICADO)
==============================================
...
- Apego Social (Percentil Estimado): PC {st.session_state.pc_social}

DEFINICIONES TÉCNICAS APLICADAS:
1. Confianza: Seguridad y respeto en la relación.
2. Comunicación: Calidad del intercambio verbal y emocional.
3. Alienación: Sentimientos de aislamiento o rechazo.
4. Total: Balance general del apego.
