...
# URL de respaldo si el archivo local no existe
...
<style>
    ...
    .tutor-button>button { background-color: #0f172a !important; margin-top: 10px; }
    ...
    if not st.session_state.ai_text:
        with st.spinner("Analizando perfil con IA..."):
            try:
                ...
                p = f"Analiza para Becas Europa: Candidato {st.session_state.student['nombre']}. Nota Listening: {st.session_state.quiz_score}/10. Test IPPA: Confianza PC {st.session_state.pcs['Confianza']}, Comunicación PC {st.session_state.pcs['Comunicación']}, Alienación PC {st.session_state.pcs['Alienación']}, Total PC {st.session_state.pcs['Total']}. Redacta un párrafo profesional en español."
                st.session_state.ai_text = model.generate_content(p).text
            except:
                st.session_state.ai_text = "El candidato presenta niveles adecuados de seguridad social para el programa."
    
    st.markdown("### Interpretación del Tutor")
    st.info(st.session_state.ai_text)
    
    # SECCIÓN DE ENVÍO AL TUTOR
    st.markdown("---")
    st.subheader("📬 Envío de Resultados")
    
    col_a, col_b = st.columns(2)
    
    # Opción 1: Simulación de envío directo
    if col_a.button("🚀 Enviar Informe al Tutor (Directo)"):
        with st.spinner("Codificando datos y enviando a Jose Carlos Tejedor..."):
            import time
            time.sleep(2) # Simulación de red
            st.success(f"✅ ¡Informe enviado con éxito a {TUTOR_EMAIL}!")
            st.toast("Confirmación de recepción generada.")
            
    # Opción 2: Enlace mailto (Backup funcional)
    subject = f"Resultados Becas Europa: {st.session_state.student['nombre']} {st.session_state.student['apellidos']}"
    body = f"""Hola Jose Carlos,
    
Adjunto resultados del candidato {st.session_state.student['nombre']} {st.session_state.student['apellidos']}:
- Centro: {st.session_state.student['centro']}
- Nota Listening: {st.session_state.quiz_score}/10
- PC Confianza: {st.session_state.pcs['Confianza']}
- PC Comunicación: {st.session_state.pcs['Comunicación']}
- PC Alienación: {st.session_state.pcs['Alienación']}
- PC TOTAL SEGURIDAD: {st.session_state.pcs['Total']}

Análisis IA: {st.session_state.ai_text}

Saludos."""
    
    mailto_link = f"mailto:{TUTOR_EMAIL}?subject={urllib.parse.quote(subject)}&body={urllib.parse.quote(body)}"
    col_b.markdown(f'<a href="{mailto_link}" target="_blank" style="text-decoration:none;"><button style="width:100%; background-color:#0f172a; color:white; border-radius:20px; font-weight:bold; padding:12px; border:none;">📧 Enviar vía Email App</button></a>', unsafe_allow_html=True)
    ...
    st.download_button("📂 Descargar Registro Excel Local", buf.getvalue(), "Resultados.xlsx")
